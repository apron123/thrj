...
if __name__ == '__main__':
    ...
    if args.demo_mode == 'on':
        ...
        movie_info_df = movie_info_df[['movie_title', 'keywords']]
        ...
        while True:
            try:
                ...
                try:
                    movie_keyword = literal_eval(movie_info_df[movie_info_df['movie_title'] == '1917']['keywords'].item())
                    movie_keyword_df = pd.DataFrame(movie_keyword, index=['cosine_score']).T
                except:
    ...
    # demo_mode off
        ...
        # 영화데이터 가져오기
        movie_info_df = pd.read_csv('./mvcrawling_utf.csv')
        ...
        try :
            ...
            # 키워드 컬럼 추가 및 csv 저장
            movie_info_df['keywords'] = keywords
            movie_info_df.to_csv("moviedata_keywords.csv", index=False, encoding='utf-8')
            print('영화 키워드 추출 완료')
        except :
